...
impl State {
    ...
    pub fn state_root(&self) -> BigUint {
        ...
        let mut leaves: Vec<BigUint> = ids.iter().map(|id| self.accounts[id].hash()).collect();
...
mod tests {
    ...
    fn test_insufficient_balance() {
        ...
        assert!(matches!(
            state.apply_tx(&tx),
            Err(RollupError::InsufficientBalance {
                available: 10,
                requested: 100
            })
        ));
    ...
    fn test_apply_batch() {
        ...
        let root_before = state.state_root();
        let make_tx = |from: u32, to: u32, amount: u64, nonce: u64| {
            let mut msg = vec![];
            msg.extend(from.to_be_bytes());
            msg.extend(to.to_be_bytes());
            msg.extend(amount.to_be_bytes());
